...
def display_model(
    ...
    if model_faces is None:
        ...
        coords_start = model_info['verts'][batch_idx][start_vertex_index].tolist()
        ax.scatter(coords_start[0], coords_start[1], coords_start[2], color="blue")

        # Add the border nodes
        for border_index in border_indices:
            coords_border = model_info['verts'][batch_idx][border_index].tolist()
            ax.scatter(coords_border[0], coords_border[1], coords_border[2], color="red")
        

        partial_mesh = Poly3DCollection(np.array(partial_verts)[partial_faces], alpha=0.2)
        partial_mesh.set_facecolor("red")
        ax.add_collection3d(partial_mesh)
        '''
